...
module.exports.notifyDeliveryCompany = async (event) => {
  ...
  // Here HTTP call to our third party delivery company to deliver the cake
  console.log('Lets imagine we have called delivery company endpoint');
  return 'done';
}
...
module.exports.orderDelivered = async (event) => {
  ...
  return deliveryManager.orderDelivered(orderId, deliveryCompanyId, orderReview).then(
    () => {
      return createResponse(200, `Order with ${orderId} was delivered sucessfully by 
...
module.exports.notifyCustomerService = async (event) => {
  console.log('Lets imagine we have called customer Service endpoint here');
}
